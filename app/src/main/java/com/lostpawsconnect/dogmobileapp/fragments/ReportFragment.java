...
public class ReportFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = FragmentReportBinding.inflate(LayoutInflater.from(requireContext()), container, false);
        loadDataFirestore();
        ...
        if (dogsList.size() > 0) {
            adapter = new ReportAdapter(requireContext(), dogsList, new ReportListener() {
                ...
                public void onClickListener(int position) {
                    ...
                    Intent intent = new Intent(mContext, ReportDogDetailsActivity.class);
    ...
    private void loadDataFirestore() {
        ...
        FirebaseFirestore.getInstance().collection("dogs_registered").whereEqualTo("userID", userID).get().addOnSuccessListener(queryDocumentSnapshots -> {
